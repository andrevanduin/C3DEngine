			...
			if (attachmentConfig.type == RenderTargetAttachmentType::Color)
			{
				...
				else
				{
					...
					if (attachmentConfig.loadOperation == RenderTargetAttachmentLoadOperation::Load)
					{
						if (doClearColor)
						{
							Logger::Warn("[RENDER_PASS] - Create() - Color attachment load operation is set to load, but is also set to clear. This combination is invalid and should not be used.");
					...
					{
						Logger::Fatal("[RENDER_PASS] - Create() - Invalid and unsupported combination of load operation ({}) and clear flags ({}) for color attachment.", attachmentDescription.loadOp, m_clearFlags);
						return false;
					}
				}
			...
			else if (attachmentConfig.type == RenderTargetAttachmentType::Depth)
			{
				...
				else
				{
					...
					if (attachmentConfig.loadOperation == RenderTargetAttachmentLoadOperation::Load)
					{
						if (doClearDepth)
						{
							Logger::Warn("[RENDER_PASS] - Create() - Depth attachment load operation set to load, but also set to clear. This combination is invalid and should not be used.");
							attachmentDescription.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
						}
						else
						{
					...
					{
						Logger::Fatal("[RENDER_PASS] - Create() - Invalid and unsupported combination of load operation ({}) and clear flags ({}) for depth attachment.", attachmentDescription.loadOp, m_clearFlags);
						return false;
					}
				}
