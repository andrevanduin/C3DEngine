...
namespace C3D
{
    ...
    bool VulkanDevice::Create(vkb::Instance instance, VulkanContext* context, VkAllocationCallbacks* callbacks)
    {
        ...
        vkb::PhysicalDevice vkbPhysicalDevice =
            selector.set_minimum_version(1, 2).set_surface(context->surface).select().value();
        ...
        VK_SET_DEBUG_OBJECT_NAME(context, VK_OBJECT_TYPE_DEVICE, logicalDevice, "Vulkan Logical Device");
        ...
        m_logger.Info("Driver Version - {}.{}.{}", VK_VERSION_MAJOR(driverVersion), VK_VERSION_MINOR(driverVersion),
                      VK_VERSION_PATCH(driverVersion));
        m_logger.Info("API Version    - {}.{}.{}", VK_VERSION_MAJOR(apiVersion), VK_VERSION_MINOR(apiVersion),
    ...
    void VulkanDevice::QuerySwapChainSupport(VkSurfaceKHR surface, VulkanSwapChainSupportInfo* supportInfo) const
    {
        ...
        if (supportInfo->formatCount != 0)
        {
            ...
            {
                supportInfo->formats =
                    Memory.Allocate<VkSurfaceFormatKHR>(MemoryType::RenderSystem, supportInfo->formatCount);
            }

            VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, surface, &supportInfo->formatCount,
                                                          supportInfo->formats))
        }

        VK_CHECK(
            vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, surface, &supportInfo->presentModeCount, nullptr))

        if (supportInfo->presentModeCount != 0)
        {
            if (!supportInfo->presentModes)
            {
                supportInfo->presentModes =
                    Memory.Allocate<VkPresentModeKHR>(MemoryType::RenderSystem, supportInfo->presentModeCount);
            }

            VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, surface, &supportInfo->presentModeCount,
                                                               supportInfo->presentModes))
        }
    }
    ...
    bool VulkanDevice::DetectDepthFormat()
    {
        ...
        constexpr VkFormat candidates[candidateCount] = { VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT,
    ...
    void VulkanDevice::WaitIdle() const { vkDeviceWaitIdle(logicalDevice); }
}  // namespace C3D
