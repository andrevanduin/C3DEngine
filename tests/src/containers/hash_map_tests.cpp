...
void HashMap::RegisterTests(TestManager* manager)
{
	...
	manager->Register(HashMapShouldSetAndGet, "You should be able to set an entry in the HashMap by key and get it with the same key.");
	manager->Register(HashMapGetShouldBeEditable, "You should be able to get and entry by key and edit it.");
	manager->Register(HashMapHasShouldWork, "HashMap Has() function should return true if key already exists and false otherwise.");
