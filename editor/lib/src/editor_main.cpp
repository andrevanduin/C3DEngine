...
#include "views/render_view_editor_world.h"
#include "views/render_view_pick.h"
#include "views/render_view_ui.h"
#include "views/render_view_wireframe.h"
#include "views/render_view_world.h"

constexpr const char* INSTANCE_NAME = "EDITOR";

Editor::Editor(C3D::ApplicationState* state) : Application(state), m_state(reinterpret_cast<EditorState*>(state)) {}

bool Editor::OnBoot()
{
    INFO_LOG("Starting the Editor.");
...
bool Editor::OnRun(C3D::FrameData& frameData)
{
...
void Editor::OnUpdate(C3D::FrameData& frameData)
{
...
bool Editor::OnPrepareRenderPacket(C3D::RenderPacket& packet, C3D::FrameData& frameData)
{
    // Get our application specific frame data
    auto appFrameData = static_cast<GameFrameData*>(frameData.applicationFrameData);
    ...
    packet.views[EDITOR_VIEW_WORLD].view = Views.Get("WORLD_VIEW");
    packet.views[EDITOR_VIEW_WORLD].geometries.SetAllocator(frameData.frameAllocator);

    packet.views[EDITOR_VIEW_EDITOR_WORLD].view = Views.Get("EDITOR_WORLD_VIEW");
    packet.views[EDITOR_VIEW_EDITOR_WORLD].geometries.SetAllocator(frameData.frameAllocator);

    packet.views[EDITOR_VIEW_WIREFRAME].view = Views.Get("WIREFRAME_VIEW");
    packet.views[EDITOR_VIEW_WIREFRAME].geometries.SetAllocator(frameData.frameAllocator);

    packet.views[EDITOR_VIEW_UI].view = Views.Get("UI_VIEW");
    packet.views[EDITOR_VIEW_UI].geometries.SetAllocator(frameData.frameAllocator);
    ...
    if (m_state->simpleScene.GetState() == SceneState::Loaded)
    {
        ...
        {
            packet.views[EDITOR_VIEW_WORLD].debugGeometries.EmplaceBack(line.GetModel(), line.GetGeometry(), INVALID_ID);
        }
        for (auto& box : m_state->testBoxes)
        {
            packet.views[EDITOR_VIEW_WORLD].debugGeometries.EmplaceBack(box.GetModel(), box.GetGeometry(), INVALID_ID);
        }
    }
    else
    {
        // Ensure that we always have at least one viewport
        packet.views[EDITOR_VIEW_WORLD].viewport = &m_state->worldViewport;
    }
    ...
    auto& editorWorldViewPacket = packet.views[EDITOR_VIEW_EDITOR_WORLD];
    ...
    // Wireframe
    {
        ...
        wireframeData.worldGeometries         = packet.views[EDITOR_VIEW_WORLD].geometries;
        wireframeData.terrainGeometries       = packet.views[EDITOR_VIEW_WORLD].terrainGeometries;

        auto& wireframeViewPacket = packet.views[EDITOR_VIEW_WIREFRAME];
    ...
    auto& uiViewPacket = packet.views[EDITOR_VIEW_UI];
...
bool Editor::OnRender(C3D::RenderPacket& packet, C3D::FrameData& frameData)
{
    ...
    // World
    C3D::RenderViewPacket* viewPacket = &packet.views[EDITOR_VIEW_WORLD];
    viewPacket->view->OnRender(frameData, viewPacket);

    // Editor world
    viewPacket = &packet.views[EDITOR_VIEW_EDITOR_WORLD];
    viewPacket->view->OnRender(frameData, viewPacket);

    // Render the wireframe view
    viewPacket = &packet.views[EDITOR_VIEW_WIREFRAME];
    viewPacket->view->OnRender(frameData, viewPacket);

    // UI
    viewPacket = &packet.views[EDITOR_VIEW_UI];
    viewPacket->view->OnRender(frameData, viewPacket);
...
bool Editor::ConfigureRenderViews() const
{
    // World View
...
bool Editor::OnButtonUp(u16 code, void* sender, const C3D::EventContext& context)
{
...
bool Editor::OnMouseMoved(u16 code, void* sender, const C3D::EventContext& context)
{
...
bool Editor::OnMouseDragged(u16 code, void* sender, const C3D::EventContext& context)
{
    u16 button = context.data.u16[0];
...
bool Editor::OnDebugEvent(const u16 code, void*, const C3D::EventContext&)
{
