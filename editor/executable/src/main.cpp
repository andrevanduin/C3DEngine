...
const char* libPath       = "EditorLib";
const char* loadedLibPath = "EditorLib_loaded";

const char* INSTANCE_NAME = "EDITOR_MAIN";
...
bool TryCopyLib()
{
    ...
    {
        ERROR_LOG("Failed to copy Game library.");
        return false;
    }

    INFO_LOG("Copied Game library: '{}' -> '{}'.", source, dest);
    return true;
}
...
bool OnWatchedFileChanged(const u16, void* sender, const C3D::EventContext& context)
{
    ...
    if (!applicationLib.Load(loadedLibPath))
    {
