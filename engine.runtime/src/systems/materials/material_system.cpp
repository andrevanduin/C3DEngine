...
#define MATERIAL_APPLY_OR_FAIL(expr)               \
    ...
    bool MaterialSystem::ApplyGlobal(u32 shaderId, const FrameData& frameData, const DirectionalLightData& dirLight, const mat4* projection,
                                     const mat4* view, const vec4* cascadeSplits, const vec3* viewPosition, const u32 renderMode) const
    {
        ...
        if (shaderId == m_terrainShaderId)
        {
            ...
            MATERIAL_APPLY_OR_FAIL(Shaders.SetUniformByIndex(m_terrainLocations.dirLight, &dirLight));
    ...
    bool MaterialSystem::ApplyPointLights(Material* material, const DynamicArray<PointLightData, LinearAllocator>& pointLights,
                                          u16 pLightsLoc, u16 numPLightsLoc) const
    {
    ...
    bool MaterialSystem::ApplyInstance(Material* material, const DirectionalLightData& dirLight,
                                       ...
                                       const bool needsUpdate) const
    {
        ...
        if (needsUpdate)
        {
            if (material->shaderId == m_pbrShaderId)
            {
                ...
                // Directional light
                MATERIAL_APPLY_OR_FAIL(Shaders.SetUniformByIndex(m_pbrLocations.dirLight, &dirLight));

                // Point lights
                if (!ApplyPointLights(material, pointLights, m_pbrLocations.pLights, m_pbrLocations.numPLights))
                {
                    return false;
                }
            }
            else if (material->shaderId == m_terrainShaderId)
            {
                ...
                // Apply point lights
                if (!ApplyPointLights(material, pointLights, m_terrainLocations.pLights, m_terrainLocations.numPLights))
                {
                    return false;
                }
            }
