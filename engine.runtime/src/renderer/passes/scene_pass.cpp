...
namespace C3D
{
    ...
    bool ScenePass::Initialize(const C3D::LinearAllocator* frameAllocator)
    {
        ...
        m_directionalLights.SetAllocator(frameAllocator);
        m_pointLights.SetAllocator(frameAllocator);
    ...
    bool ScenePass::Prepare(const Viewport& viewport, Camera* camera, FrameData& frameData, Scene& scene, u32 renderMode,
                            ...
                            ShadowMapCascadeData* cascadeData)
    {
        ...
        auto frustum = Frustum(cameraPos, forward, right, up, viewport.GetNearClip(), viewport.GetFarClip(), viewport.GetFov(),
        ...
        // Get all directional lights from the scene
        scene.QueryDirectionalLights(frameData, m_directionalLights);

        // Get all the point lights from the scene
        scene.QueryPointLights(frameData, m_pointLights);
    ...
    bool ScenePass::Execute(const C3D::FrameData& frameData)
    {
        ...
        if (!m_terrains.Empty())
        {
            ...
            // Apply globals
            if (!Materials.ApplyGlobal(m_terrainShader->id, frameData, m_directionalLights[0], &projectionMatrix, &viewMatrix,
                                       &m_cascadeSplits, &viewPosition, m_renderMode))
            {
            ...
            for (const auto& terrain : m_terrains)
            {
                ...
                if (!Materials.ApplyInstance(m, m_directionalLights[0], m_pointLights, frameData, needsUpdate))
                {
        ...
        if (!m_geometries.Empty())
        {
            ...
            // Apply globals
            if (!Materials.ApplyGlobal(m_pbrShader->id, frameData, m_directionalLights[0], &projectionMatrix, &viewMatrix, &m_cascadeSplits,
                                       &viewPosition, m_renderMode))
            {
