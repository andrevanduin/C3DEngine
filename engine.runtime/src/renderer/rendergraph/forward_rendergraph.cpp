...
namespace C3D
{
    ...
    bool ForwardRendergraph::OnPrepareRender(FrameData& frameData, const Viewport& currentViewport, Camera* currentCamera, Scene& scene,
                                             u32 renderMode, const DynamicArray<DebugLine3D>& debugLines,
                                             const DynamicArray<DebugBox3D>& debugBoxes)
    {
        ...
        if (scene.GetState() == SceneState::Loaded)
        {
            ...
            m_shadowMapPass.Prepare(frameData, currentViewport, currentCamera, scene);
