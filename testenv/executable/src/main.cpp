...
bool OnWatchedFileChanged(const u16, void* sender, const C3D::EventContext& context)
{
	...
	engine->OnApplicationLibraryReload(applicationLib.Create(applicationState));
...
{
	engine->GetSystem<EventSystem>(EventSystemType).Register(EventCodeWatchedFileChanged, 
		[engine](const u16 code, void*, const EventContext& context)
		{
