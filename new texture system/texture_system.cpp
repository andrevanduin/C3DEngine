...
namespace C3D
{
    ...
    TextureHandle TextureSystem::Acquire(const String& name, bool autoRelease)
    {
        ...
        {
            INFO_LOG("Called for: '{}' texture. Which does not exist yet. So let's load it from disk.");

            return Create(name, autoRelease);
        }

        INFO_LOG("Called for: '{}' texture. Which already exists. Incrementing reference count.", name);
        auto& ref = m_registeredTextures.Get(name);
        ref.referenceCount++;

        return m_nameToHandleMap.Get(name);
    }
