...
namespace C3D
{
    CameraSystem::CameraSystem(const SystemManager* pSystemsManager)
        : SystemWithConfig(pSystemsManager, "CAMERA_SYSTEM"), m_cameras(nullptr)
    {}
    ...
    Camera* CameraSystem::Acquire(const char* name)
    {
        ...
        if (id == INVALID_ID_U16)
        {
            ...
            {
                m_logger.Error(
                    "Acquire() - Failed to acquire new slot. Adjust camera system config to allow more (returned "
                    "nullptr).");
                return nullptr;
            }
