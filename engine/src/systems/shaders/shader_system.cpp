...
namespace C3D
{
    ShaderSystem::ShaderSystem(const SystemManager* pSystemsManager)
        : SystemWithConfig(pSystemsManager, "SHADER_SYSTEM"), m_currentShaderId(0)
    {}
    ...
    bool ShaderSystem::ApplyGlobal() { return Renderer.ShaderApplyGlobals(&m_shaders.GetByIndex(m_currentShaderId)); }
